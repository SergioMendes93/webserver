...
func worker(requests int, completeCh chan time.Duration) {
	for i := 0; i < requests; i++ {
		...
		cmd := exec.Command("docker","-H", "tcp://10.5.60.2:2377","run", "-itd", "-c", "2", "-m", "400000", "-e", "affinity:requestclass==4", "-e", "affinity:requesttype==job","-e", "affinity:makespan==3000", "-e", "affinity:port==9000", "ubuntu")
